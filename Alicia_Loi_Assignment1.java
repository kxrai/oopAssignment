...
public class Alicia_Loi_Assignment1 {
    ...
    /**
     * Reads first line so loop can start on second
     * @param questionDataArray Array for the question data
     * @param questionFile file path to txt
     */
    public static void readQuestionFile(String[][] questionDataArray, String questionFile) {
        BufferedReader br = null;
        try{	
            ...
            while (contentLine != null) {
                //System.out.println(contentLine);
                questionDataArray[count] = contentLine.split(",");
                count++;
                contentLine = br.readLine();
            }
        }
    ...
    /**
     * Reads the data from the ansewer file that the user inputs and cleans the data up by ignoring the excess spaces
     * @param studentDataArray Array for student data
     * @param file file path to txt for answer data files
     */
    public static void readAnswerData(String[] studentDataArray, String file) {
        BufferedReader br = null;
        try{	
            br = new BufferedReader(new FileReader(file));	
            int count = 0;
    ...
    /**
     * Reads the answer file that the user inputs
     * Will output as an 2D Array
     * @param answerDataArray Array for answer data
     * @param answerFile File path for txt file
     */
    public static void readAnswerFile(String[][] answerDataArray, String answerFile) {
        BufferedReader br = null;
        try{	
            br = new BufferedReader(new FileReader(answerFile));	
            int count = 0;
            br.readLine(); // reads the first line so that the while loop can start on the second line
            String contentLine = br.readLine();
            while (contentLine != null) {
                //System.out.println(contentLine);
                answerDataArray[count] = contentLine.split(",");
                count++;
                contentLine = br.readLine();
            }
        }
